...
def upsample_argmax(output, size=(512,512)):
    '''
    Upsample the score map from model
    :param output: N*C*H*W
    :param size: (dim*dim)
    :return: (dim*dim) segmentation map
    '''

    interp = nn.UpsamplingBilinear2d(size=size)
    seg_map = interp(output).cpu().data.numpy()[0]
